      ...
      link: function (scope, element, attrs, linkCtrl, snowowlService) {
        ...
        function drawConceptDiagram (concept, div, options) {
            ...
            if (options.selectedView == "stated") {
                $.each(concept.outboundRelationships, function(i, field) {
                ...
                if (concept.outboundRelationships) {
                    $.each(concept.outboundRelationships, function (i, field) {
                        if (field.active == true) {
                            if (field.typeId == 116680003) {
                                svgIsaModel.push(field);
                            } else {
                                svgAttrModel.push(field);
                            }
...
//                } else {
                    sctClass = "sct-defined-concept";
//                }
                var rectParent = drawSctBox(svg, x, y, relationship.destinationName, relationship.destinationId, sctClass);
...
//                }
                if (relationship.group == 0) {
                    var rectAttr = drawSctBox(svg, x, y, relationship.destinationName, relationship.destinationId, "sct-attribute");
                    connectElements(svg, circle2, rectAttr, 'center', 'left');
                    var rectTarget = drawSctBox(svg, x + rectAttr.getBBox().width + 50, y, relationship.destinationName, relationship.destinationId, sctClass);
                    ...
                    if (relationship.group > maxRoleNumber) {
            ...
            for (var i = 1; i <= maxRoleNumber; i++) {
                ...
                $.each(svgAttrModel, function(m, relationship) {
                    if (relationship.group == i) {
//                        if (relationship.target.definitionStatus == "Primitive") {
//                            sctClass = "sct-primitive-concept";
//                        } else {
                            sctClass = "sct-defined-concept";
//                        }
                        var rectRole = drawSctBox(svg, x + 85, y - 18, relationship.destinationName, relationship.destinationId,"sct-attribute");
                        connectElements(svg, conjunctionNode, rectRole, 'center', 'left');
                        var rectRole2 = drawSctBox(svg, x + 85 + rectRole.getBBox().width + 30, y - 18, relationship.destinationName, relationship.destinationId, sctClass);
